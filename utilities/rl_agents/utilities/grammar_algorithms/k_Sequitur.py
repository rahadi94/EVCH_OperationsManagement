...
class k_Sequitur(object):
    ...
    def generate_action_grammar(self, actions):
        ...
        assert not isinstance(
            actions[0], list
        ), "Should be 1 long list of actions - {}".format(actions[0])
        ...
        new_actions, all_rules, rule_usage, rules_episode_appearance_count = (
            self.discover_all_rules_and_new_actions_representation(actions)
        )
        action_usage = self.extract_action_usage_from_rule_usage(rule_usage, all_rules)
        rules_episode_appearance_count = self.extract_action_usage_from_rule_usage(
            rules_episode_appearance_count, all_rules
        )
    ...
    def discover_all_rules_and_new_actions_representation(self, actions):
        ...
        while new_actions != current_actions:
            ...
            new_actions, rules_usage_count = self.convert_a_string_using_reverse_rules(
                current_actions, reverse_rules, rules_episode_appearance_tracker
            )
    ...
    def generate_1_layer_of_rules(self, string):
        ...
        assert (
            string[-1] == self.end_of_episode_symbol
        ), "Final element of string must be self.end_of_episode_symbol {}".format(
            string
        )
        ...
        for ix in range(len(string) - 1):
            ...
            if pairs_of_symbols[pair] >= self.k:
                ...
                if (
                    pair not in rules.values()
                    and self.end_of_episode_symbol not in pair
                ):
    ...
    def extract_action_usage_from_rule_usage(self, rule_usage, all_rules):
        ...
        for key in rule_usage.keys():
            action_usage[self.convert_symbol_to_raw_actions(key, all_rules)] = (
                rule_usage[key]
            )
        return action_usage

    def convert_a_string_using_reverse_rules(
        self, string, reverse_rules, rules_episode_appearance_tracker
    ):
