...
class SAC(Base_Agent):
    ...
    def __init__(self, config):
        ...
        assert (
            self.config.hyperparameters["Actor"]["final_layer_activation"] != "Softmax"
        ...
        #     try:
        #         self.critic_local.load_state_dict(torch.load(f'Results/critic_{self.config.name}'))
        #         self.critic_local.eval()
        #         self.critic_local_2.load_state_dict(torch.load(f'Results/critic_2_{self.config.name}'))
        ...
        if config.learnt_network:
            try:
                self.actor_local.load_state_dict(
                    torch.load(f"data/output/actor_{self.config.name}")
                )
        ...
        if self.automatic_entropy_tuning:
            self.target_entropy = -torch.prod(
                torch.Tensor(self.environment.action_space.shape).to(self.device)
            ).item()  # heuristic value from the paper
        ...
        self.action_range = [
            config.environment.action_space.low,
            config.environment.action_space.high,
        ]
        self.action_scale = torch.FloatTensor(
            (config.environment.action_space.high - config.environment.action_space.low)
            / 2.0
        )
        ...
        self.n_step = 1
        self.action_memory = np.zeros([self.n_step, self.action_size])
        self.state_memory = np.zeros([self.n_step, self.state_size])
        self.reward_memory = np.zeros(self.n_step)
        self.step_counter = 0
        self.objective_function = [0]
        self.learning_rate_critic = self.hyperparameters["Critic"]["learning_rate"]
        self.learning_rate_actor = self.hyperparameters["Actor"]["learning_rate"]

    def update_lr(self, new_objective, episode=1):
        a = 0.1
        new_objective = new_objective[0]
        if max(self.objective_function) >= 10000:
            if max(self.objective_function) >= max(new_objective, new_objective + 500):
                self.learning_rate_critic *= a
                self.learning_rate_actor *= a
                self.critic_optimizer = torch.optim.Adam(
                    self.critic_local.parameters(),
        ...
        self.objective_function.append(new_objective)

        # a = 0.90
        # b = 50
        # if episode > b:
        #     self.critic_optimizer = torch.optim.Adam(self.critic_local.parameters(),
        #                                              lr=self.hyperparameters["Critic"]["learning_rate"]*a**(episode-b), eps=1e-10)
        #     self.critic_optimizer_2 = torch.optim.Adam(self.critic_local_2.parameters(),
        #                                                lr=self.hyperparameters["Critic"]["learning_rate"]*a**(episode-b),
        #                                                eps=1e-10)
        #     self.actor_optimizer = torch.optim.Adam(self.actor_local.parameters(),
        #                                             lr=self.hyperparameters["Actor"]["learning_rate"]*a**(episode-b), eps=1e-10)
        #
        #     print(f'Learning rate: {self.hyperparameters["Actor"]["learning_rate"]*a**(episode-b)}')
    ...
    def save_result(self):
        ...
        if self.episode_number == 1 or not self.do_evaluation_iterations:
            ...
            self.rolling_results.extend(
                [
                    np.mean(
                        self.game_full_episode_scores[-1 * self.rolling_score_window :]
                    )
                    for _ in range(TRAINING_EPISODES_PER_EVAL_EPISODE)
                ]
            )
    ...
    def step(self):
        ...
        eval_ep = (
            ...
            and self.do_evaluation_iterations
        )
    ...
    def calculate_critic_losses(
        self, state_batch, action_batch, reward_batch, next_state_batch, mask_batch
    ):
        ...
        with torch.no_grad():
            ...
            next_q_value = reward_batch + (1.0 - mask_batch) * self.hyperparameters[
                "discount_rate"
            ] * (min_qf_next_target)
    ...
    def save_models(self):
        if not self.do_evaluation_iterations:
            # print(f'saving the results for {self.config.name}_agent')
