...
if method == "optimisation":
    lay_out = (
        pd.read_csv("investment_results_Facility_KoeBogen_2019-06-03_TR_PRICES.csv")
...
def evaluation(i):
    ...
    for j in range(0, 1):
        if method == "Manual":
            ...
            NUM_CHARGER["slow_one"] += 101
            NUM_CHARGER["slow_two"] += 0
            NUM_CHARGER["slow_four"] += 0
            TRANSFORMER_NUM += 2
            PV_CAPA += 0
            STORAGE_CAPA += 400
        if method == "optimisation":
        ...
        for week in test_weeks:
            ...
            results = [f"GA", f"state{j}", f"week{week_number}"]
            (
                new_objective,
                new_objective_investment,
                new_objective_operations,
            ...
            inner_energy_consumption += new_energy_consumption
            week_number += 1
        ...
        energy_consumption = inner_energy_consumption / len(test_weeks)
        KPIs = dict(
            objective=objective,
