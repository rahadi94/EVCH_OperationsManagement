from Deep_Reinforcement_Learning_Algorithms_with_PyTorch_master.exploration_strategies.Base_Exploration_Strategy import (
...
class Epsilon_Greedy_Exploration(Base_Exploration_Strategy):
    ...
    def __init__(self, config):
        ...
        if "random_episodes_to_run" in self.config.hyperparameters.keys():
            self.random_episodes_to_run = self.config.hyperparameters[
    ...
    def perturb_action_for_exploration_purposes(self, action_info):
        ...
        if (random.random() > epsilon or turn_off_exploration) and (
            episode_number >= self.random_episodes_to_run
        ):
            return torch.argmax(action_values).item()
        return np.random.randint(0, action_values.shape[1])

    def get_updated_epsilon_exploration(self, action_info, epsilon=0.5):
        """Gets the probability that we just pick a random action. This probability decays the more episodes we have seen"""
        episode_number = action_info["episode_number"]
        epsilon_decay_denominator = self.config.hyperparameters[
    ...
    def calculate_epsilon_with_cyclical_strategy(self, episode_number):
        ...
        max_epsilon = 0.5
        min_epsilon = 0.001
        increment = (max_epsilon - min_epsilon) / float(
            self.exploration_cycle_episodes_length / 2
        )
        cycle = [
            ix for ix in range(int(self.exploration_cycle_episodes_length / 2))
