...
from Deep_Reinforcement_Learning_Algorithms_with_PyTorch_master.exploration_strategies.Epsilon_Greedy_Exploration import (
...
class DQN(Base_Agent):
    ...
    def __init__(self, config):
        ...
        #     # self.q_network_local.load_state_dict(torch.load(f'Results/q_network'))
    ...
    def step(self, env):
        ...
        while not self.done:
            ...
            self.conduct_action(self.action, env)
    ...
    def learn(self, experiences=None):
        ...
        if self.config.evaluation_after_training:
            return
    ...
    def compute_q_values_for_next_states(self, next_states):
        ...
        Q_targets_next = (
            self.q_network_local(next_states).detach().max(1)[0].unsqueeze(1)
        )
    ...
    def time_for_q_network_to_learn(self):
        ...
        return (
            self.right_amount_of_steps_taken()
    ...
    def save_models(self):
        torch.save(
            self.q_network_local.state_dict(), f"Results/q_network_{self.config.name}"
        )
