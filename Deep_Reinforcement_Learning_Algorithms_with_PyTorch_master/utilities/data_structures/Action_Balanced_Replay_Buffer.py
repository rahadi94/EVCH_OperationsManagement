...
class Action_Balanced_Replay_Buffer(Replay_Buffer):
    ...
    def __init__(self, buffer_size, batch_size, seed, num_actions):
        ...
        self.memories = {
            action: deque(maxlen=self.buffer_size_per_memory)
    ...
    def calculate_batch_sizes_per_action(self, batch_size):
        ...
        current_batch_size = np.sum(
            [batch_per_action[k] for k in range(self.num_actions)]
        )
    ...
    def sample_each_action_equally(self, batch_per_action):
        ...
        for action in range(self.num_actions):
            ...
            if action_memory_size >= batch_size_for_action:
                ...
                print(
                    "Memory size {} vs. required batch size {}".format(
    ...
    def sample_experiences_with_certain_actions(
        self, allowed_actions, num_all_actions, required_batch_size
    ):
        ...
        assert (
            abs(states.shape[0] - required_batch_size) <= 0.05 * required_batch_size
        ), "{} vs. {}".format(states.shape[0], required_batch_size)
