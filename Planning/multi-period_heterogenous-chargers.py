...
def planning(facility, date, plug=4, adoption=1, service_level=1):
    ...
    t_0 = 7
    t_end = 22
    dfs, loads, pv_profile = data_preparation(facility, date, t_0=t_0, size=100)
    ...
    for s in year_range:
        ...
        C_grid[s] = 0.001  # (GRID_PRICES[s*2]+GRID_PRICES[s*2+1]) / 2 / 365 / 20
        C_grid_fixed[s] = 50000 / 365 / 20  # TODO: do we keep it?
    ...
    for k in space_range:
        mdl.add_constraint(
            mdl.sum(
                x_standard[k, s, l] + x_standard[k, s, l]
                for l in connector_range
                for s in year_range
            )
            <= 1,
        ...
        for s in year_range:
            for t in time_range:
                for j in vv_range[s, t]:
                    mdl.add_constraint(
                        h[k, j, s, t] <= w[k, j, s] * U[j, s, t] * 1000, "C15"
                    )
                    mdl.add_constraint(
    ...
    investment_results.to_csv(f"investment_results_{facility}_{date}.csv")
